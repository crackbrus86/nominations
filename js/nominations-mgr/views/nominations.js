...
class Nominations extends React.Component {
		...
		services
			...
			.then((data) => {
				const nextLifters = JSON.parse(data).map((x) => {
					const wClass = this.state.weightClasses.find(
						(w) => w.id === x.wId
					);
	...
	evalCompStatus() {
		...
		var compStatus = '';
		var p = new Date(info.startDate).setDate(
			new Date(info.startDate).getDate() - 30
		);
		var f = new Date(info.startDate).setDate(
			new Date(info.startDate).getDate() - 15
		);
		var mc = new Date(info.startDate).setDate(
			new Date(info.startDate).getDate()
		);
		if (+new Date() <= p) {
			compStatus = 'p';
	...
	changeNom(field, value) {
		...
		if (temp['type'] === 'lifter')
			temp['total'] =
				parseFloat(temp.squat) +
				parseFloat(temp.benchpress) +
				parseFloat(temp.deadlift);
	...
	saveNom() {
		...
		if (this.state.nomination.type === 'lifter') {
			if (this.state.nomination.id) {
				services
					.updateLifterNominationById(this.state.nomination)
					.then(() => {
						this.closeNom();
						this.setState({ isLoading: false });
						this.showInform(
							'Номінацію спортсмена було успішно оновлено'
						);
						this.getLifterNominations();
						this.getRefereeNominations();
					});
				...
				services
					.insertLifterNomination(this.state.nomination)
					.then(() => {
						this.closeNom();
						this.setState({ isLoading: false });
						this.showInform(
							'Спортсмена було успішно додано до номінації'
						);
						this.getLifterNominations();
						this.getRefereeNominations();
					});
			...
			if (this.state.nomination.id) {
				services
					.updateOfficialNominationById({...this.state.nomination, wcBusy: this.state.selectedWClasses })
					.then(() => {
						this.closeNom();
						this.setState({ isLoading: false });
						this.showInform(
							'Номінацію судді було успішно оновлено'
						);
						this.getLifterNominations();
						this.getRefereeNominations();
					});
				...
				services
					.insertOfficialNomination({ ...this.state.nomination, wcBusy: this.state.selectedWClasses })
					.then(() => {
						this.closeNom();
						this.setState({ isLoading: false });
						this.showInform(
							'Суддю було успішно додано до номінації'
						);
						this.getLifterNominations();
						this.getRefereeNominations();
					});
	...
	changeNomStatus(id, value) {
		this.setState({ isLoading: true });
		services
			.checkNominationStatusById({ id: id, status: value })
			.then(() => {
				this.setState({ isLoading: false });
				this.getLifterNominations();
				this.getRefereeNominations();
	...
	getWeightCategories(gender) {
		const allCategories = wCl.weightClasses.filter(
			(c) => c.gender === gender
		);
		const openClasses = allCategories
			.filter((c) => c.division === 'open')
			.sort((a, b) => a.sortOrder - b.sortOrder);
		const subJuniorClasses = allCategories
			.filter((c) => c.division === 'subjuniors')
			.sort((a, b) => a.sortOrder - b.sortOrder);
		this.setState({
			weightClasses: allCategories,
			wc: openClasses,
			subwc: subJuniorClasses,
		});
	}
	...
	render() {
		...
		var addLifterButton =
			this.state.compStatus === 'p' ? (
				<span>
					<img
						src="../wp-content/plugins/nominations/images/nom_add.png"
						alt=""
						title="Додати спортсмена"
						onClick={this.setNomination.bind(this, 'lifter')}
					/>
				</span>
			) : (
				''
			);
		var addOfficialButton =
			this.state.compStatus === 'p' || this.state.compStatus === 'f' ? (
				<span>
					<img
						src="../wp-content/plugins/nominations/images/nom_add_ref.png"
						alt=""
						title="Додати офіційну особу"
						onClick={this.setNomination.bind(this, 'official')}
					/>
				</span>
			) : (
				''
		...
		return (
			<div>
				...
				/>
				<RefGrid
					nominations={this.state.rNominations}
					game={this.state.compInfo}
					onOfficialEdit={this.onOfficialEdit}
					onDelete={this.onDelete}
					onShowTooltip={this.onShowTooltip}
				/>
